        ...
        <div className='flex ml-6 flex-wrap'>
            {loading ? (
                ...
                sortedUnits.filter(e => e.subject === subject).map(e => (
                    <div key={e.unit[0].number} className="max-w-sm w-72 p-6 m-3 text-center bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                        <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Unit - {e.unit[0].number}</h5>
                        <hr className="my-2" />
                        <h5 className="mb-2 text-xl font-bold tracking-tight text-green-500 dark:text-white">{e.unit[0].name}</h5>
                        <div className="mb-3">
